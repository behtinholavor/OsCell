...
begin
  ...
  begin
    Caixa := 0;
    MainView.Mensagem.Aviso('Senha inválida!', True, edtSenha);
  end;
end;
...
begin
  ...
  if Caixa > 0 then
  begin
    if MainView.Mensagem.Pergunta('Deseja realmente fechar o caixa Nº ' +Caixa.ToString()+ ' ?') then
    begin
...
begin
  ...
  begin
    ...
    begin
      MainView.Mensagem.Aviso('O status '+qryOs.FieldByName('status').AsString+' não é permitido para recebimento em caixa!', False);
      qryOs.Close;
      actAtualizarExecute(Sender);
    end;
    edtOs.Clear;
...
begin
  inherited;
  PesquisaView  := Pesquisa('Pesquisa de Ordem de Serviços', 'os');
...
begin
  ...
  if OS > 0 then
  begin
    if MainView.Mensagem.Pergunta('Deseja autenticar a OS Nº ' +OS.ToString()+ ' ?') then
    begin
      AQuery.Connection := qryAux.Connection;
      ...
      AQuery.ParamByName('descricao').AsString  := 'RECEBIMENTO DA OS Nº '+OS.ToString() +' NO CAIXA Nº '+Caixa.ToString();
      ...
      edtOs.Clear;
      qryOs.Close;
      MainView.Mensagem.Aviso('Atenticação realizada com sucesso!', False);
      actAtualizarExecute(Sender);
    end;
  end;
  FreeAndNil(AQuery);
...
begin
  ...
  begin
    if MainView.Mensagem.Pergunta('Salvar movimento no caixa Nº ' +Caixa.ToString()+ ' ?') then
    begin
      AQuery.Connection := qryAux.Connection;
      AQuery.SQL.Clear;
      AQuery.SQL.Add('INSERT INTO                 ');
