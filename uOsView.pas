...
//  if (Value > 0) and (pgcOs.ActivePage = tabPesquisa) then
  if (Value > 0) then
  begin
    pnlEdicao.Caption   := 'Ordem de Serviços  ['+IntToStr(Value)+']';
  ...
  begin
    pnlEdicao.Caption   := 'Ordem de Serviços';
    dtDataOs.Date       := Today;
    edtFoneOs.Clear;
    edtNomeOs.Clear;
    Cliente             := 0;
...
begin
  Col := 0;
  Idx := 0;
  AGrid.Columns.Items[0].Title.Caption := 'OS Nº';
...
begin
  ...
  begin
    if cdsOs.FieldByName('idstatus').AsInteger in [0,4,5] then
      MainView.Mensagem.Aviso('Não é possível alterar OS com status '+cdsOs.FieldByName('status').AsString, True, grdPesquisa);
    ...
    MainView.Mensagem.Aviso('Alteração não permitida, pesquisa vazia!');
...
begin
  ...
  begin
//    prt.Add('#######################################');
//    prt.Add('OS Nº    :' + qryOs.FieldByName('idservico').AsString);
...
begin
  ...
  AQuery := TFDQuery.Create(nil);
  if MainView.Mensagem.Pergunta('Deseja modificar o status da OS Nº ' +Ordem.ToString()+ ' para '+Msg+'?') then
  begin
    AQuery.Connection := qryAux.Connection;
    AQuery.SQL.Clear;
